...
class ModelProject(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.adaptive_max_pool1d(x, 1).squeeze(dim=-1)
        x = self.projection(x.unsqueeze(-1)).squeeze()
        return x

def pointMLPProject(num_classes=40, **kwargs) -> ModelProject:
    return ModelProject(points=1024, class_num=num_classes, embed_dim=64, groups=1, res_expansion=1.0,
